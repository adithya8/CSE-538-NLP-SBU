...
class ParsingSystem:
    ...
    def apply(self, configuration: Configuration, transition: str) -> Configuration:
        ...
        if(transition[0:2]=='L('):
            ...
            configuration.add_arc(configuration.get_stack(0), configuration.get_stack(1) , transition[2:-1])
            configuration.remove_second_top_stack()
        elif(transition[0:2]=='R('):
            #Right Reduce
            configuration.add_arc(configuration.get_stack(1), configuration.get_stack(0) , transition[2:-1])
